...
public class VendingMachineService : IVending
{
    ...
    // Lista för att hålla reda på köpta produkter
    private readonly List<Product> purchasedProducts = new List<Product>();
    ...
    public void Purchase(int productId)
    {
        ...
        if (moneyPool >= product.Cost)
        {
            ...
            // Lägg till den köpta produkten till listan
            purchasedProducts.Add(product);
            Console.WriteLine($"Purchased {product.Name}. {product.Use()}");
        }
        else
        {
            Console.WriteLine("Not enough money. Please insert more money. Please!");
        }
    }
    ...
    public Dictionary<int, int> EndTransaction()
    {
        ...
        // Beräkna total antal varor och total pris
        int totalProducts = purchasedProducts.Count;
            ...
            try
            {
                switch (choice)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        // Show all products before purchasing
                        // Hämtar listan över alla produkter.
                        var allProducts = vendingMachine.ShowAll();
                        // Skriver ut alla produkter till konsolen innan kunden ombeds ange produkt-ID.
